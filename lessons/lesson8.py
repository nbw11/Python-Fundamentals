...
# Removing items from a list by remove(), pop(), and clear()
# The remove function will remove from specified value from the list.
def simple_remove():
    colors.remove('green')
    for color in colors:
        print(color)
...
# not picking an index will the the last item
def simple_pop():
    colors.pop(5)
    for value in colors:
        print(value)
...
# list operation: Concatenation, repeat, membership, and iterate
# concatenation
def simple_join():
    values1 = [1, 2, 3, 4]
    values2 = [5, 6, 7, 8]
...
# Joining set collectionsby update, union and the | operator
even = {2, 4, 6, 8, 10}
odd = {1, 3, 5, 7, 9}
tens = {10, 20, 30, 40, 50}
